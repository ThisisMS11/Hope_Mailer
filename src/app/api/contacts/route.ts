...
export const POST = async (req: NextRequest) => {
    ...
    try {
        ...
        const { firstName, lastName, gender, mobile, linkedIn, email, company, position, experience } = body;
        ...
        // Validate input
        if (!firstName || !lastName || !gender || !mobile || !email || !company || !position || !userId) {
        ...
        if (!["male", "female", "not_know"].includes(gender)) {
            ...
            return makeResponse(400, false, "Gender must be 'male', 'female', or 'not_know'.", null);
        }
        ...
        return makeResponse(500, false, "Server error. Please try again later.", null);
    }
};
...
export const GET = async (req: NextRequest) => {
    try {
        ...
        if (id) {
            ...
            return makeResponse(200, true, "Contact retrieved successfully.", contact);
        } else {
            const contacts = await prisma.contact.findMany();
            logger.info("Contacts retrieved successfully.");
            return makeResponse(200, true, "Contacts retrieved successfully.", contacts);
        }
    } catch (error) {
...
export const PUT = async (req: NextRequest) => {
    try {
        ...
        return makeResponse(200, true, "Contact updated successfully.", updatedContact);
    } catch (error) {
